...
describe('Spinner', () => {
  ...
  test('applies correct dimensions for each size', () => {
    ...
    expect(screen.getByRole('status', { name: 'Loading' })).toHaveClass('w-4', 'h-4')

    rerender(<Spinner size="md" />)
    expect(screen.getByRole('status', { name: 'Loading' })).toHaveClass('w-6', 'h-6')

    rerender(<Spinner size="lg" />)
    expect(screen.getByRole('status', { name: 'Loading' })).toHaveClass('w-10', 'h-10')
  })
  ...
  test('merges className safely while preserving required base styles', () => {
    ...
    const spinner = screen.getByRole('status', { name: 'Loading' })
    expect(spinner).toHaveClass('inline-flex', 'justify-center', 'items-center')
