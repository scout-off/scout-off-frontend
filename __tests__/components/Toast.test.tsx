...
function ToastTestApp() {
  ...
  return (
    <div>
      <button onClick={() => toast.show({ message: "Success message", variant: "success" })}>
        Show Success
      </button>
      <button onClick={() => toast.show({ message: "Error message", variant: "error" })}>
        Show Error
      </button>
      <button onClick={() => toast.show({ message: "Info message", variant: "info" })}>
        Show Info
      </button>
      <button onClick={() => toast.show({ message: "Warning message", variant: "warning" })}>
        Show Warning
      </button>
    </div>
  );
...
describe("Toast notifications", () => {
  ...
  it("renders variant-specific icon and border for success", () => {
    ...
    const toast = screen.getAllByRole("alert").find((node) => node.textContent?.includes("Success message"));
  ...
  it("limits visible toasts to a maximum of 3 items", () => {
    ...
    const toastMessages = screen.queryAllByRole("alert").map((element) => element.textContent ?? "");

    expect(toastMessages).toHaveLength(3);
    expect(toastMessages.some((text) => text.includes("Initial toast"))).toBe(false);
    expect(toastMessages.some((text) => text.includes("Success message"))).toBe(false);
    expect(toastMessages.some((text) => text.includes("Warning message"))).toBe(true);
    expect(toastMessages.some((text) => text.includes("Error message"))).toBe(true);
    expect(toastMessages.some((text) => text.includes("Info message"))).toBe(true);
  });
});
